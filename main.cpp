...
void triangle (Vec3i*tr,float *z_buffer,TGAImage &image,TGAColor color) {
    ...
    for (int x = border_min.x; x <= border_max.x; ++x) {
        for (int y = border_min.y; y <= border_max.y; ++y) {
            ...
            float H = 0;
            for (int j = 0; j < 3; ++j)     H += bc_screen.z * tr[j].z;
...
void Render(TGAImage &image) {
    ...
    for (int i=0; i<model.nfaces(); i++) { 
        ...
        for (int j=0; j<3; j++) { 
            ...
            screen_coords[j] = Vec3i((world_coords.x+1.)*width/2., (world_coords.y+1.)*height/2.,(world_coords.z)); 
        ...
        if (light_intensity > 0) {
            ...
            triangle(tr, z_buffer,image, TGAColor{(uint8_t)(255*light_intensity), (uint8_t)(255*light_intensity), (uint8_t)(255*light_intensity), 255}); 
        }
        // delete []_temp;
    }  
    delete []z_buffer;
